# ElevenLabs TTS Prompting Guide

## Overrides for this project

These rules take precedence over the general guidance below when generating tags for `/video-audio-tags`:

- **No pause tags**: Do not use `[short pause]`, `[long pause]`, or similar pause tags. The json-video-player inserts silence automatically at scene boundaries.
- **No text modification**: Step 4 of the workflow below suggests adding emphasis via capitalization, punctuation, and ellipses. Do NOT do this. Speech text must remain unchanged — only add bracketed tags around it.
- **Non-verbal tags**: Available (`[sighs]`, `[chuckles]`, `[laughing]`, etc.) but use sparingly. Only when they reflect a genuinely natural vocal moment in educational narration — not as filler.
