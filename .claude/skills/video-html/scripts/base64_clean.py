...
This is the inverse of embed-data.py:
  embed-data.py   →  local file paths → base64 data URIs   (production step)
...
-----------
    ...
    already been through the embed-data.py production step. After the LLM finishes
    editing, re-run embed-data.py to restore the base64 payloads before playback.
"""
