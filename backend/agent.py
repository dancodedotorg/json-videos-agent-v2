...
Skills loaded (all from backend/skills/):
  lesson-plan, video-init, video-script, video-html, video-audio-tags,
...
video_skills = [
    load_skill_from_dir(SKILLS_DIR / "lesson-plan"),
...
root_agent = Agent(
    ...
    instruction=(
        ...
        "  What video are you working on? If you tell me the /s/ slug (e.g. aif1-v2-2025) and\n"
        "  the lesson number (e.g. lesson 2) of a published unit, I can fetch the public-facing\n"
        "  lesson resources and walk you through planning the video(s).\n\n"
        "  Or if this isn't tied to a Code.org unit, just give me a name for the lesson\n"
        "  and I'll ask you what source materials you want to base the video on.\n\n"
        "  I can also:\n"
        ...
        "Entry point — two paths based on whether a Code.org unit slug is provided:\n\n"
        "PATH A: User provides a unit slug + lesson (the normal case)\n"
        "When the user provides a /s/ slug (e.g. aif1-v2-2025) and a lesson number or name:\n"
        "1. Call ground_lesson(unit=<unit>, lesson=<lesson>) to fetch and ground source materials.\n"
        "   Handle each status:\n\n"
        ...
        "     Read sources.csv with read_file, then show the user the full list:\n"
        "     'Here's what I found for this lesson — does this look right? Let me know if you\n"
        "     want to add, remove, or change any entries. If vocabulary, objectives, or a Google\n"
        "     Slides link are missing, let me know and I can add them.'\n"
        "     Wait for confirmation and make any adjustments with write_file. Then call\n"
        ...
        "3. Load and run the lesson-plan skill. The user will choose video type(s), review the\n"
        "   recommended plan, and confirm video names. lesson-plan initializes all video folders.\n"
        "4. After lesson-plan confirms the plan, share the preview URL pattern:\n"
        "   'You can preview each video as it builds at:\n"
        "   /preview/<unit>/<lesson_slug>/<video_name> — it updates after each stage.'\n"
        "5. For each video in the plan, run: video-script → video-html → video-audio-tags →\n"
        "   video-audio → video-assemble. Check in with the user between videos.\n\n"
        "PATH B: User does NOT provide a unit slug (standalone)\n"
        "When the user says they don't have a Code.org unit, or provides materials directly:\n"
        "1. Ask: 'What slug would you like to use for this lesson? (short, lowercase-hyphenated,\n"
        "   e.g. intro-to-loops)'\n"
        "2. Call ground_lesson('standalone', <slug>). This will return 'needs_sources_csv'.\n"
        "3. Ask the user what materials to include: Google Slides URL, any objectives,\n"
        ...
        "4. Call load_lesson_sources('standalone', <slug>).\n"
        "5. Skip lesson-plan — go directly to video-init to set mode, objectives, and video name.\n"
        "   After video-init confirms the video name, share the preview URL.\n"
        "6. Continue: video-script → video-html → video-audio-tags → video-audio → video-assemble.\n\n"
        ...
        "Common patterns:\n"
        "- Missing source files or load_lesson_sources returns error: call ground_lesson for\n"
        "  the unit and lesson, then call load_lesson_sources again once grounding completes.\n"
